...
const PLAIN_CODES = new Set([
  ...
  "interview_exhausted",
  "nothing_asked",
  // Пределы AI — два разных отказа: лимит запросов проходит через минуту, а
  // исчерпанный бюджет — нет, и «попробуйте позже» на второй читалось бы как
  // ложное обещание.
  "ai_rate_limited",
  "ai_budget_exhausted",
  // Публичная ссылка у проекта одна: повторное «поделиться» — не поломка, а
  // ответ «уже есть».
  "share_link_exists",
  // Отказы движка правок. Обычно до них не доходит — значения приходят из
  // списков, — но при рассинхроне сборки клиента с сервером человек должен
  // увидеть слова, а не «неизвестная ошибка».
  "unknown_status",
  "unknown_risk",
  "unknown_operation",
  "revision_not_found",
